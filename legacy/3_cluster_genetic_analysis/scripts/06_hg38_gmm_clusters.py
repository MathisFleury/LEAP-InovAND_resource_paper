#!/usr/bin/env python3
# =============================================================================
# 06 - hg38 Carrier Frequencies & Odds Ratios by GMM Cluster
# =============================================================================
# Same as 02_gmm_clusters.py but uses hg38 carrier annotations.
#
# Input:  carrier_annotations_hg38.tsv (2_genetic_analysis outputs)
#         df_multi_dataset_with_clusters.csv
# Output: figures/tables in outputs/figures_gmm_hg38/
# =============================================================================
...
from _config import (
    ...
    FIGURES_GMM_HG38_DIR, TABLES_GMM_HG38_DIR,
...
os.makedirs(FIGURES_GMM_HG38_DIR, exist_ok=True)
os.makedirs(TABLES_GMM_HG38_DIR, exist_ok=True)
...
def main():
    ...
    print("Cluster_with_NT distribution:")
    ...
    print("\n=== hg38 cluster carrier frequencies (constrained) ===")
    plot_carrier_frequencies(
        df, "Cluster_with_NT", cols_constrained,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "hg38_gmm_constraint_freq.pdf", ylim=(0, 1),
        figures_dir=FIGURES_GMM_HG38_DIR,
    )
    print("\n=== hg38 cluster odds ratios (constrained) ===")
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", cols_constrained,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "hg38_gmm_constraint_or.pdf",
        figures_dir=FIGURES_GMM_HG38_DIR, tables_dir=TABLES_GMM_HG38_DIR,
    )

    print("\n=== hg38 cluster carrier frequencies (all genes) ===")
    plot_carrier_frequencies(
        df, "Cluster_with_NT", cols_all,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, labels_all_no_diag,
        "hg38_gmm_allgenes_freq.pdf",
        figures_dir=FIGURES_GMM_HG38_DIR,
    )
    print("\n=== hg38 cluster odds ratios (all genes) ===")
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", cols_all,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, labels_all_no_diag,
        "hg38_gmm_allgenes_or.pdf",
        figures_dir=FIGURES_GMM_HG38_DIR, tables_dir=TABLES_GMM_HG38_DIR,
    )

    print("\n=== hg38 PGS odds ratios by GMM cluster ===")
    if os.path.exists(PGS_FILE):
        ...
        if len(df_ors) > 0:
            ...
            df_ors.to_csv(os.path.join(TABLES_GMM_HG38_DIR, "hg38_gmm_pgs_or.csv"), index=False)
            plot_pgs_forest(df_ors, PALETTE_CLUSTERS, LABELS_PGS,
                            "hg38_gmm_pgs_or.pdf", FIGURES_GMM_HG38_DIR)
    else:
        print(f"Warning: PGS file not found ({PGS_FILE}), skipping")

    print(f"\n=== Done. Outputs in: {FIGURES_GMM_HG38_DIR} ===")
