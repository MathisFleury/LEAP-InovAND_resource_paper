...
tables_dir <- file.path(base_dir, "tables")   # reads 01's *_full_data.csv
output_dir <- file.path(base_dir, "figures")  # writes the schaefer PDFs
...
load_autism_td_connectivity_data <- function(connectivity_type) {
  if (connectivity_type == "all") {
    file_path <- file.path(tables_dir, "autism_vs_td_hyperconnectivity_full_data.csv")
  } else {
    file_path <- file.path(tables_dir, paste0("autism_vs_td_", connectivity_type, "connectivity_full_data.csv"))
  }
  ...
  if (connectivity_type == "all") {
    hyper_file <- file.path(tables_dir, "autism_vs_td_hyperconnectivity_full_data.csv")
    hypo_file  <- file.path(tables_dir, "autism_vs_td_hypoconnectivity_full_data.csv")
  ...
  } else if ("t_stat" %in% names(data)) {
    # legacy combined file carried t_stat; split by sign. The NBS full_data files
    # (from 04) are ALREADY per-direction (source,target only) — no t_stat, so
    # skip the filter (applying data$t_stat>0 on a NULL column drops all rows).
