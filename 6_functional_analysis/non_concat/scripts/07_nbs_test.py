...
def main() -> int:
    ...
    np.savez(OUT_DIR / "nbs_null_max.npz", **null_max)
    ...
    for d in ("hypo", "hyper"):
        ...
        res.to_csv(OUT_DIR / f"nbs_components_{d}.csv", index=False)
        ...
        fig.tight_layout()
        fig.savefig(OUT_DIR / f"nbs_null_distribution_{d}.pdf", dpi=300, bbox_inches="tight")
        plt.close(fig)
        ...
        fig.tight_layout()
        fig.savefig(OUT_DIR / f"nbs_block_counts_{d}.pdf", dpi=300, bbox_inches="tight")
        plt.close(fig)
