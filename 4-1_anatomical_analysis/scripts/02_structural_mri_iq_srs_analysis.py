...
def load_and_prepare_data():
    ...
    df_mri = load_mri(merge_clusters_cols=['PopulationS1', 'total_IQ', 'performance_IQ', 'SRS_tscore', 'vabsabcabc_standard'])

    df_autism = df_mri[df_mri['PopulationS1'] == 'Autism'].copy()
    ...
    thickness_cols = [col for col in df_autism.columns if 'thickness' in col.lower()]
    area_cols = [col for col in df_autism.columns if 'area' in col.lower() and 'thickness' not in col.lower()]
    volume_cols = [col for col in df_autism.columns if 'volume' in col.lower()]
    ...
    print(f"Dataset: {len(df_autism)} autism individuals with MRI data")
    return df_autism
