...
def make_violin(df):
    ...
    ax.set_yticks(range(len(order))); ax.set_yticklabels([labels[g] for g in order], fontsize=16)
    ax.invert_yaxis(); ax.set_xlim(0, 1.0)
    ax.tick_params(axis="both", labelsize=16)
    for s in ("top", "right"): ax.spines[s].set_visible(False)
    draw(axall, ["All"], lambda g: df["mean_fd"].dropna().values)
    axall.set_yticks([0]); axall.set_yticklabels(["All"], fontsize=16); axall.set_xlim(0, 1.0)
    axall.tick_params(axis="both", labelsize=16)
    for s in ("top", "right"): axall.spines[s].set_visible(False)
    axall.set_xlabel("framewise displacement — FD (mm)", fontsize=18)
    fig.tight_layout()
