...
# Group sizes written by 01_anatomical_mri_autism_td_analysis.py — appended to
# every figure subtitle so the N is visible on the plot.
n_suffix <- ""
counts_path <- file.path(input_dir, "group_counts.csv")
if (file.exists(counts_path)) {
  gc <- read.csv(counts_path)
...
if (!is.null(cortical_thickness_data)) {
  ...
  p1 <- cortical_thickness_data %>%
    ...
    brain_map_theme +
    labs(title = "Cortical Thickness: Autism vs TD", subtitle = paste0("t-statistics (FDR-corrected p < 0.05 highlighted)", n_suffix))
  ...
  p2 <- cortical_thickness_data %>%
    ...
    brain_map_theme +
    labs(title = "Cortical Thickness: Autism vs TD", subtitle = paste0("t-statistics (FDR-corrected p < 0.05 highlighted with thick black border)", n_suffix))
...
if (!is.null(cortical_area_data)) {
  ...
  p3 <- cortical_area_data %>%
    ...
    scale_fill_continuous_divergingx(palette = 'RdBu', mid = 0, rev = TRUE,
                                     limits = c(-t_stat_max, t_stat_max), name = "t-statistic") +
    brain_map_theme +
    labs(title = "Surface Area: Autism vs TD", subtitle = paste0("t-statistics (FDR-corrected p < 0.05 highlighted)", n_suffix))
  ...
  p4 <- cortical_area_data %>%
    ...
    scale_colour_identity() + scale_size_identity() +
    brain_map_theme +
    labs(title = "Surface Area: Autism vs TD", subtitle = paste0("t-statistics (FDR-corrected p < 0.05 highlighted with thick black border)", n_suffix))
...
if (!is.null(subcortical_data)) {
  ...
  p5 <- subcortical_data %>%
    ...
    scale_fill_continuous_divergingx(palette = 'RdBu', mid = 0, rev = TRUE,
                                     limits = c(-t_stat_max, t_stat_max), name = "t-statistic") +
    brain_map_theme +
    labs(title = "Subcortical Volumes: Autism vs TD", subtitle = paste0("t-statistics (FDR-corrected p < 0.05 highlighted)", n_suffix))
  ...
  p6 <- subcortical_data %>%
    ...
    scale_colour_identity() + scale_size_identity() +
    brain_map_theme +
    labs(title = "Subcortical Volumes: Autism vs TD", subtitle = paste0("t-statistics (FDR-corrected p < 0.05 highlighted with thick black border)", n_suffix))
