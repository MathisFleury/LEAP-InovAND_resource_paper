...
_SECTION_DIR = os.path.dirname(_SCRIPT_DIR)  # legacy/4_anatomical_analysis/
_LIB_DIR = os.path.normpath(os.path.join(_SECTION_DIR, "..", "..", ".."))  # parent of the repo
...
EEG_MRI_RESULTS = os.path.join(_LIB_DIR, 'eeg_mri-pipeline', 'results')
DATA_IMAG2GENET_DIR = os.path.join(_LIB_DIR, 'imaging2genet')
...
def compute_cohort_specific_statistics(df, metric_name, atlas_regions, atlas_type='dk'):
    ...
    for cohort_name in ['INOVAND', 'LEAP']:
        ...
        t_stat_df = pd.DataFrame(t_stat_results)
        if len(t_stat_df) == 0 or 'p_val' not in t_stat_df.columns:
            print(f"  Warning: no usable regions for {cohort_name} ({metric_name}/{atlas_type}); skipping")
            continue
...
def main():
    ...
    # Persist group sizes so the R plotter can print them on the figures.
    pd.DataFrame([{
        "n_autism": int((df_mri["PopulationS1"] == "Autism").sum()),
        "n_nt": int((df_mri["PopulationS1"] == "NT").sum()),
    }]).to_csv(os.path.join(R_INPUT_DIR, "group_counts.csv"), index=False)

    # (metric column suffix, atlas region dict, atlas_type). Two 'volume' rows:
    # DK cortical volume (-> t_stat_anat_dk_volume...) and ASEG subcortical
    # volume (-> t_stat_anat_aseg_volume...). The atlas_type disambiguates the
    # output filename so both coexist; the DK row enables the 4-panel figure.
    metrics = [
        ('thickness', data_dk, 'dk'),
        ('area', data_dk, 'dk'),
        ('volume', data_dk, 'dk'),       # cortical volume (NEW)
        ('volume', data_aseg, 'aseg'),   # subcortical volume
    ]

    for metric_name, atlas, atlas_type in metrics:
        print(f"\n{'=' * 60}")
        print(f"Processing {metric_name.upper()} ({atlas_type})")
        print(f"{'=' * 60}")
        ...
        compute_t_statistics(df_metric, metric_name, atlas, atlas_type=atlas_type)

        if 'cohort' in df_metric.columns:
            compute_cohort_specific_statistics(df_metric, metric_name, atlas, atlas_type)
