#!/usr/bin/env python3
# =============================================================================
# 07 - hg38 Carrier Frequencies & Odds Ratios by Legacy Cluster (NT = C1 only)
# =============================================================================
# Same as 05_hg38_legacy_clusters.py but uses only NT subjects whose legacy
# cluster assignment is C1 as the reference group.
#
# Input:  carrier_annotations_hg38.tsv (2_genetic_analysis outputs)
#         cluster_assignments.csv (1_clustering outputs)
# Output: figures/tables in outputs/figures_legacy_hg38_nt_c1/
# =============================================================================
...
from _config import (
    CARRIER_ANNOTATIONS_HG38, CLUSTERS_LEGACY_FILE,
    INDIVIDUALS_METRICS, DIAG_FILE, PGS_FILE,
    FIGURES_LEGACY_HG38_NTC1_DIR, TABLES_LEGACY_HG38_NTC1_DIR,
...
os.makedirs(FIGURES_LEGACY_HG38_NTC1_DIR, exist_ok=True)
os.makedirs(TABLES_LEGACY_HG38_NTC1_DIR, exist_ok=True)
...
def main():
    ...
    plot_carrier_frequencies(
        ...
        "hg38_legacy_nt_c1_constraint_freq.pdf", ylim=(0, 1),
        figures_dir=FIGURES_LEGACY_HG38_NTC1_DIR,
    )
    print("\n=== hg38 cluster odds ratios (constrained, NT=C1) ===")
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", cols_constrained,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "hg38_legacy_nt_c1_constraint_or.pdf",
        figures_dir=FIGURES_LEGACY_HG38_NTC1_DIR, tables_dir=TABLES_LEGACY_HG38_NTC1_DIR,
    )
    ...
    plot_carrier_frequencies(
        ...
        "hg38_legacy_nt_c1_allgenes_freq.pdf",
        figures_dir=FIGURES_LEGACY_HG38_NTC1_DIR,
    )
    ...
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", cols_all,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, labels_all_no_diag,
        "hg38_legacy_nt_c1_allgenes_or.pdf",
        figures_dir=FIGURES_LEGACY_HG38_NTC1_DIR, tables_dir=TABLES_LEGACY_HG38_NTC1_DIR,
    )

    print("\n=== hg38 PGS odds ratios by legacy cluster (NT=C1) ===")
    if os.path.exists(PGS_FILE):
        ...
        if len(df_ors) > 0:
            ...
            df_ors.to_csv(os.path.join(TABLES_LEGACY_HG38_NTC1_DIR, "hg38_legacy_nt_c1_pgs_or.csv"), index=False)
            plot_pgs_forest(df_ors, PALETTE_CLUSTERS, LABELS_PGS,
                            "hg38_legacy_nt_c1_pgs_or.pdf", FIGURES_LEGACY_HG38_NTC1_DIR)
    else:
        print(f"Warning: PGS file not found ({PGS_FILE}), skipping")

    print(f"\n=== Done. Outputs in: {FIGURES_LEGACY_HG38_NTC1_DIR} ===")
