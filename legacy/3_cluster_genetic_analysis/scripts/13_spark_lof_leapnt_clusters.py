#!/usr/bin/env python3
# =============================================================================
# 13 - SPARK LoF Carrier Freq & OR with LEAP-InovAND NT (GMM Clusters)
# =============================================================================
# Supplementary analysis: SPARK autism clusters (GMM) tested against
...
from _config import (
    ...
    FIGURES_SPARK_LEAPNT_DIR, TABLES_SPARK_LEAPNT_DIR,
...
os.makedirs(FIGURES_SPARK_LEAPNT_DIR, exist_ok=True)
os.makedirs(TABLES_SPARK_LEAPNT_DIR, exist_ok=True)
...
def main():
    ...
    clusters_spark = clusters[clusters["Cohort"] == "SPARK"][["ID", "Cluster"]].copy()
    ...
    # Carry barcode for NT so PGS can be merged later
    spark_subset = df_spark_pop[["ID", "population", "Cluster"] + lof_carrier_cols].copy()
    spark_subset["barcode"] = spark_subset["ID"]  # SPARK: barcode == ID
    ...
    plot_carrier_frequencies(
        ...
        "SPARK_leapnt_lof_constraint_freq_population.pdf", ylim=(0, 0.8),
        figures_dir=FIGURES_SPARK_LEAPNT_DIR,
    )
    compute_and_plot_odds_ratios(
        df, "population", cols_constrained,
        PALETTE_POP, ORDER_POP_OR, LABELS_SPARK_LOF_CONSTRAINED,
        "SPARK_leapnt_lof_constraint_or_population.pdf",
        reference_group="NT",
        tables_dir=TABLES_SPARK_LEAPNT_DIR, figures_dir=FIGURES_SPARK_LEAPNT_DIR,
    )
    ...
    plot_carrier_frequencies(
        ...
        "SPARK_leapnt_lof_allgenes_freq_population.pdf",
        figures_dir=FIGURES_SPARK_LEAPNT_DIR,
    )
    compute_and_plot_odds_ratios(
        df, "population", cols_all,
        PALETTE_POP, ORDER_POP_OR, LABELS_SPARK_LOF_ALL,
        "SPARK_leapnt_lof_allgenes_or_population.pdf",
        reference_group="NT",
        tables_dir=TABLES_SPARK_LEAPNT_DIR, figures_dir=FIGURES_SPARK_LEAPNT_DIR,
    )
    ...
    plot_carrier_frequencies(
        ...
        "SPARK_leapnt_lof_constraint_freq_autism_vs_nt.pdf", ylim=(0, 0.8),
        figures_dir=FIGURES_SPARK_LEAPNT_DIR,
    )
    compute_and_plot_odds_ratios(
        df, "PopulationS1_simple", cols_constrained,
        PALETTE_POP, ORDER_POP_SIMPLE_OR, LABELS_SPARK_LOF_CONSTRAINED,
        "SPARK_leapnt_lof_constraint_or_autism_vs_nt.pdf",
        reference_group="NT",
        tables_dir=TABLES_SPARK_LEAPNT_DIR, figures_dir=FIGURES_SPARK_LEAPNT_DIR,
    )
    ...
    plot_carrier_frequencies(
        ...
        "SPARK_leapnt_lof_allgenes_freq_autism_vs_nt.pdf",
        figures_dir=FIGURES_SPARK_LEAPNT_DIR,
    )
    compute_and_plot_odds_ratios(
        df, "PopulationS1_simple", cols_all,
        PALETTE_POP, ORDER_POP_SIMPLE_OR, LABELS_SPARK_LOF_ALL,
        "SPARK_leapnt_lof_allgenes_or_autism_vs_nt.pdf",
        reference_group="NT",
        tables_dir=TABLES_SPARK_LEAPNT_DIR, figures_dir=FIGURES_SPARK_LEAPNT_DIR,
    )
    ...
    if n_clustered > 0:
        ...
        plot_carrier_frequencies(
            ...
            "SPARK_leapnt_lof_cluster_constraint_freq.pdf", ylim=(0, 1),
            figures_dir=FIGURES_SPARK_LEAPNT_DIR,
        )
        compute_and_plot_odds_ratios(
            df_cluster, "Cluster_with_NT", cols_constrained,
            PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_SPARK_LOF_CONSTRAINED,
            "SPARK_leapnt_lof_cluster_constraint_or.pdf",
            reference_group="NT",
            figures_dir=FIGURES_SPARK_LEAPNT_DIR, tables_dir=TABLES_SPARK_LEAPNT_DIR,
        )
        ...
        plot_carrier_frequencies(
            ...
            "SPARK_leapnt_lof_cluster_allgenes_freq.pdf",
            figures_dir=FIGURES_SPARK_LEAPNT_DIR,
        )
        compute_and_plot_odds_ratios(
            df_cluster, "Cluster_with_NT", cols_all,
            PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_SPARK_LOF_ALL,
            "SPARK_leapnt_lof_cluster_allgenes_or.pdf",
            reference_group="NT",
            figures_dir=FIGURES_SPARK_LEAPNT_DIR, tables_dir=TABLES_SPARK_LEAPNT_DIR,
        )
        ...
        if os.path.exists(PGS_FILE) and pgs_available:
            ...
            # Fill NT PGS values from LEAP-InovAND where SPARK PGS is missing
        ...
        if pgs_available:
            ...
            if len(df_ors) > 0:
                ...
                df_ors.to_csv(
                    os.path.join(TABLES_SPARK_LEAPNT_DIR, "SPARK_leapnt_pgs_or_clusters.csv"), index=False
                )
                plot_pgs_forest(df_ors, PALETTE_CLUSTERS, LABELS_PGS,
                                "SPARK_leapnt_cluster_pgs_or.pdf", FIGURES_SPARK_LEAPNT_DIR)
        else:
    ...
    print(f"\n=== Done. Outputs in: {FIGURES_SPARK_LEAPNT_DIR} ===")
