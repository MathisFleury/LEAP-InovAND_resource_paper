...
# This step writes only tables (CSVs); no plots.
OUTPUT_DIR = os.path.join(_BASE_DIR, 'tables')
...
def load_and_prepare_data():
    ...
    # Diagnosis from the curated clinical merge carried in the preprocessing
    # output (population_group), instead of re-merging the paper PopulationS1.
    # Cluster labels are still attached (left join) for any cluster-aware use.
    df_clusters['ID'] = df_clusters['ID'].astype(str)
    df_merged = df_fmri.merge(df_clusters[['ID', 'Cluster']], on='ID', how='left')
