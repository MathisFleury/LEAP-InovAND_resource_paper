#!/usr/bin/env python3
# =============================================================================
# 08 - hg38 Carrier Frequencies & Odds Ratios by GMM Cluster (NT = C1 only)
# =============================================================================
# Same as 06_hg38_gmm_clusters.py but uses only NT subjects whose Cluster_GMM
# assignment is C1 as the reference group.
#
# Input:  carrier_annotations_hg38.tsv (2_genetic_analysis outputs)
#         df_multi_dataset_with_clusters.csv
# Output: figures/tables in outputs/figures_gmm_hg38_nt_c1/
# =============================================================================
...
from _config import (
    CARRIER_ANNOTATIONS_HG38, CLUSTERS_GMM_FILE,
    INDIVIDUALS_METRICS, DIAG_FILE, PGS_FILE,
    FIGURES_GMM_HG38_NTC1_DIR, TABLES_GMM_HG38_NTC1_DIR,
...
os.makedirs(FIGURES_GMM_HG38_NTC1_DIR, exist_ok=True)
os.makedirs(TABLES_GMM_HG38_NTC1_DIR, exist_ok=True)
...
def main():
    print(f"Loading hg38 carrier annotations from: {CARRIER_ANNOTATIONS_HG38}")
    df = pd.read_table(CARRIER_ANNOTATIONS_HG38, low_memory=False)
    ...
    print(f"Loading GMM clusters from: {CLUSTERS_GMM_FILE}")
    clusters = pd.read_csv(CLUSTERS_GMM_FILE, low_memory=False)
    df = df.merge(clusters[["ID", "Cluster_GMM"]], how="left", on="ID")

    clinic = pd.read_table(INDIVIDUALS_METRICS, low_memory=False)
    df = df.merge(clinic[["ID", "Relation_to_proposant"]], how="left", on="ID")

    # NT only if Cluster_GMM == C1
    def assign_cluster_nt(row):
        if (row["PopulationS1"] == "NT"
                and row.get("Relation_to_proposant") == "participant"
                and row.get("Cluster_GMM") == "C1"):
            return "NT"
        elif row["PopulationS1"] == "IDD" and row.get("Relation_to_proposant") == "participant":
            return "IDD"
        elif pd.isna(row.get("Cluster_GMM")):
            return np.nan
        elif row["Cluster_GMM"] == "C1" and row["PopulationS1"] == "Autism":
            return "C1"
        elif row["Cluster_GMM"] == "C2" and row["PopulationS1"] == "Autism":
            return "C2"
        elif row["Cluster_GMM"] == "C3" and row["PopulationS1"] == "Autism":
            return "C3"
        return np.nan

    df["Cluster_with_NT"] = df.apply(assign_cluster_nt, axis=1)
    print("Cluster_with_NT distribution (NT = C1 only):")
    ...
    cols_constrained = [c for c in COLS_DELLOF_CONSTRAINED if c in df.columns]
    cols_all = [c for c in COLS_DELLOF_ALL if c in df.columns and c != "diag_genetic"]
    labels_all_no_diag = {k: v for k, v in LABELS_ALL_GENES.items() if k != "diag_genetic"}

    print("\n=== hg38 cluster carrier frequencies (constrained, NT=C1) ===")
    plot_carrier_frequencies(
        df, "Cluster_with_NT", cols_constrained,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "hg38_gmm_nt_c1_constraint_freq.pdf", ylim=(0, 1),
        figures_dir=FIGURES_GMM_HG38_NTC1_DIR,
    )
    print("\n=== hg38 cluster odds ratios (constrained, NT=C1) ===")
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", cols_constrained,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "hg38_gmm_nt_c1_constraint_or.pdf",
        figures_dir=FIGURES_GMM_HG38_NTC1_DIR, tables_dir=TABLES_GMM_HG38_NTC1_DIR,
    )

    print("\n=== hg38 cluster carrier frequencies (all genes, NT=C1) ===")
    plot_carrier_frequencies(
        df, "Cluster_with_NT", cols_all,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, labels_all_no_diag,
        "hg38_gmm_nt_c1_allgenes_freq.pdf",
        figures_dir=FIGURES_GMM_HG38_NTC1_DIR,
    )
    print("\n=== hg38 cluster odds ratios (all genes, NT=C1) ===")
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", cols_all,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, labels_all_no_diag,
        "hg38_gmm_nt_c1_allgenes_or.pdf",
        figures_dir=FIGURES_GMM_HG38_NTC1_DIR, tables_dir=TABLES_GMM_HG38_NTC1_DIR,
    )

    print("\n=== hg38 PGS odds ratios by GMM cluster (NT=C1) ===")
    if os.path.exists(PGS_FILE):
        ...
        if len(df_ors) > 0:
            ...
            df_ors.to_csv(os.path.join(TABLES_GMM_HG38_NTC1_DIR, "hg38_gmm_nt_c1_pgs_or.csv"), index=False)
            plot_pgs_forest(df_ors, PALETTE_CLUSTERS, LABELS_PGS,
                            "hg38_gmm_nt_c1_pgs_or.pdf", FIGURES_GMM_HG38_NTC1_DIR)
    else:
        print(f"Warning: PGS file not found ({PGS_FILE}), skipping")

    print(f"\n=== Done. Outputs in: {FIGURES_GMM_HG38_NTC1_DIR} ===")
