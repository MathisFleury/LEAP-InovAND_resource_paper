#!/usr/bin/env python3
# =============================================================================
# 04 - Carrier Frequencies & Odds Ratios by GMM Cluster (NT restricted to C1)
# =============================================================================
# Same as 02_gmm_clusters.py but uses only NT subjects whose Cluster_GMM
...
# Output: cluster-level figures/tables in outputs/figures_gmm_nt_c1/
# =============================================================================
...
from _config import (
    ...
    FIGURES_GMM_NTC1_DIR, TABLES_GMM_NTC1_DIR,
...
os.makedirs(FIGURES_GMM_NTC1_DIR, exist_ok=True)
os.makedirs(TABLES_GMM_NTC1_DIR, exist_ok=True)
...
def main():
    ...
    # --- Build Cluster_with_NT: NT only if Cluster_GMM == C1 ---
    ...
    # --- Constrained ---
    print("\n=== Cluster-level carrier frequencies (constrained, NT=C1) ===")
    plot_carrier_frequencies(
        df, "Cluster_with_NT", COLS_DELLOF_CONSTRAINED,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "gmm_nt_c1_constraint_freq.pdf", ylim=(0, 1),
        figures_dir=FIGURES_GMM_NTC1_DIR,
    )
    print("\n=== Cluster-level odds ratios (constrained, NT=C1) ===")
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", COLS_DELLOF_CONSTRAINED,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "gmm_nt_c1_constraint_or.pdf",
        figures_dir=FIGURES_GMM_NTC1_DIR, tables_dir=TABLES_GMM_NTC1_DIR,
    )
    ...
    print("\n=== Cluster-level carrier frequencies (all genes, NT=C1) ===")
    plot_carrier_frequencies(
        df, "Cluster_with_NT", cols_no_diag,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, labels_no_diag,
        "gmm_nt_c1_allgenes_freq.pdf",
        figures_dir=FIGURES_GMM_NTC1_DIR,
    )
    print("\n=== Cluster-level odds ratios (all genes, NT=C1) ===")
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", cols_no_diag,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, labels_no_diag,
        "gmm_nt_c1_allgenes_or.pdf",
        figures_dir=FIGURES_GMM_NTC1_DIR, tables_dir=TABLES_GMM_NTC1_DIR,
    )

    # --- PGS ---
    print("\n=== PGS odds ratios by cluster (NT=C1) ===")
    if os.path.exists(PGS_FILE):
        ...
        if len(df_ors) > 0:
            ...
            df_ors.to_csv(os.path.join(TABLES_GMM_NTC1_DIR, "gmm_nt_c1_pgs_or.csv"), index=False)
            plot_pgs_forest(df_ors, PALETTE_CLUSTERS, LABELS_PGS,
                            "gmm_nt_c1_pgs_or.pdf", FIGURES_GMM_NTC1_DIR)
    else:
        print(f"Warning: PGS file not found ({PGS_FILE}), skipping")

    print(f"\n=== Done. Outputs in: {FIGURES_GMM_NTC1_DIR} ===")
