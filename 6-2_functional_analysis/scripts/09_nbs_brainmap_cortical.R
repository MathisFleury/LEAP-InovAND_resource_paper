...
#
# Inputs per variant dir figures/nbs_wholegroup_<v>/:
#   nbs_edges.csv, nbs_summary.csv, autism_vs_nt_nbs_subcortical_<dir>.png
# =============================================================================
suppressMessages({
...
fig_dir     <- file.path(section_dir, "outputs", "figures")
...
# variants = env VARIANTS (comma-sep) or auto-detected nbs_wholegroup_* dirs.
env_v <- Sys.getenv("VARIANTS", "")
if (nzchar(env_v)) {
  VARIANTS <- trimws(strsplit(env_v, ",")[[1]])
} else {
  VARIANTS <- sub("^nbs_wholegroup_", "",
                  basename(Sys.glob(file.path(fig_dir, "nbs_wholegroup_*"))))
}
VARIANTS <- VARIANTS[vapply(VARIANTS, function(v)
  file.exists(file.path(fig_dir, paste0("nbs_wholegroup_", v), "nbs_edges.csv")), logical(1))]
...
count_cortical <- function(variant, direction) {
  fp <- file.path(vdir(variant), "nbs_edges.csv")
...
sub_inset <- function(variant, direction) {
  fp <- file.path(vdir(variant), sprintf("autism_vs_nt_nbs_subcortical_%s.png", direction))
