...
# 9_loeuf_combined_brain_figure.R (itself adapted from 07/13). Each figure
...
# 5_loeuf_mri_correlations_hg38.py (Pearson r + BH-FDR within each file).
#   Fill    = Pearson correlation (-log10 LOEUF vs MRI metric)
#   Outline = FDR q < 0.05 (black), per pathway x feature
#
# Input  : <base>/tables_genetics_hg38/loeuf_mri_<pathway>_<feature>.csv
# Output : <base>/figures_genetics_hg38/combined_loeuf_<pathway>_brain_map.pdf
# where <base> = outputs (default) or $LOEUF_OUT_DIR if set.
# =============================================================================
...
input_dir  <- file.path(section_dir, out_base, "tables_genetics_hg38")
output_dir <- file.path(section_dir, out_base, "figures_genetics_hg38")
...
# --- Helpers (same conventions as 13_/09_) -----------------------------------
...
load_panel <- function(short, feat, atlas) {
  fp <- file.path(input_dir, sprintf("loeuf_mri_%s_%s.csv", short, feat))
...
make_panel <- function(d, title, atlas, lim) {
  ...
  # name, so leftover region/hemisphere/mri_col cols would blank the aseg map.
  d <- dplyr::select(d, label, correlation, outline_color, outline_size)
  if (atlas == "dk") {
    p <- ggseg(d, mapping = aes(fill = correlation, col = outline_color, size = outline_size),
               atlas = "dk", position = "stacked")
  } else {
    p <- ggseg(d, mapping = aes(fill = correlation, colour = outline_color, size = outline_size),
               atlas = aseg)
  }
  p +
    scale_fill_continuous_divergingx(palette = "RdBu", mid = 0, rev = TRUE,
                                     limits = c(-lim, lim), oob = scales::squish,
                                     na.value = "grey90", name = "Pearson r") +
...
global_lim <- if (!is.na(SCALE_MAX)) SCALE_MAX else {
  vals <- unlist(lapply(cache, function(pd) lapply(pd, function(d)
    if (is.null(d)) NA_real_ else max(abs(d$correlation), na.rm = TRUE))))
...
for (pw in PATHWAYS) {
  ...
  combined <- wrap_plots(panels, ncol = 1) +
    plot_annotation(
      ...
      subtitle = "Pearson r fill · FDR q < 0.05 outlined in black",
  ...
  out_pdf <- file.path(output_dir, sprintf("combined_loeuf_%s_brain_map.pdf", pw$short))
  ggsave(out_pdf, plot = combined, width = 8, height = 5.2 * length(panels) + 1,
         limitsize = FALSE)
  cat(sprintf("Saved: %s  (lim=%.3f)\n", basename(out_pdf), global_lim))
}

cat(sprintf("\nCombined LOEUF correlation figures written to: %s\n", output_dir))
