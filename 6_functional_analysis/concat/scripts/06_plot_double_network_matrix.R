...
# tables_dir/figures_dir split by default; the nbs_double re-run (run_functional_analysis.py)
# overrides both to the same flat dir since 04 already wrote its input there unsplit.
TABLES_DIR  <- Sys.getenv("AUTISM_TD_TABLES_DIR",  file.path(IO_DIR, "tables"))
FIGURES_DIR <- Sys.getenv("AUTISM_TD_FIGURES_DIR", file.path(IO_DIR, "figures"))
...
plot_double <- function(conn_type) {
  fp <- file.path(TABLES_DIR, sprintf("autism_vs_td_%sconnectivity_full_data.csv", conn_type))
  ...
  out <- file.path(FIGURES_DIR, sprintf("autism_vs_td_%s_double_matrix_final.pdf", conn_type))
  ggsave(out, combined, width = 15, height = 7)  # base pdf device (Helvetica core font)
  write.csv(cnt, file.path(TABLES_DIR, sprintf("autism_vs_td_%s_raw_count_matrix.csv", conn_type)))
  write.csv(prop, file.path(TABLES_DIR, sprintf("autism_vs_td_%s_proportion_matrix.csv", conn_type)))
