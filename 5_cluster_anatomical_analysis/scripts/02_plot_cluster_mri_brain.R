# =============================================================================
# Cluster MRI Brain Visualizations using ggseg
# =============================================================================
#
# For each EEG cluster (C1, C2, C3) and each MRI metric (thickness, area,
# volume), loads pre-computed t-statistic CSVs (cluster vs pooled TD) and
# renders brain surface / subcortical plots using ggseg.
#
...
#'
#' ``d_max_override`` sets the symmetric colorbar limit; when NULL (legacy
#' behavior) we fall back to the per-plot max(|d|). The main loop now passes
#' a per-metric global max so all three clusters share a common scale and
#' nothing is clipped.
...
for (cfg in metrics_config) {
  ...
  # Pre-load every cluster's data for this metric so we can use a single
  # colorbar scale across the three clusters (= max |d| observed across all
  # three).  This keeps cross-cluster comparison legible while ensuring no
  # ROI is clipped to NA.
  ...
  if (!is.null(d_max_metric)) {
    d_max_metric <- max(d_max_metric, 0.1)   # avoid degenerate near-zero scale
