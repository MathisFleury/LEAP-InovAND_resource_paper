#!/usr/bin/env python3
# =============================================================================
# 01 - Carrier Frequencies & Odds Ratios by Legacy Cluster (1_clustering)
# =============================================================================
# Uses cluster assignments produced by the 1_clustering pipeline
# (1_clustering/outputs/tables/cluster_assignments.csv).
#
...
from _config import (
    ...
    FIGURES_LEGACY_DIR, TABLES_LEGACY_DIR,
...
os.makedirs(FIGURES_LEGACY_DIR, exist_ok=True)
os.makedirs(TABLES_LEGACY_DIR, exist_ok=True)
...
def main():
    ...
    # --- Load legacy cluster assignments ---
    ...
    plot_carrier_frequencies(
        ...
        "legacy_cluster_constraint_freq.pdf", ylim=(0, 1),
        figures_dir=FIGURES_LEGACY_DIR,
    )
    ...
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", COLS_DELLOF_CONSTRAINED,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "legacy_cluster_constraint_or.pdf",
        figures_dir=FIGURES_LEGACY_DIR, tables_dir=TABLES_LEGACY_DIR,
    )
    ...
    plot_carrier_frequencies(
        ...
        "legacy_cluster_allgenes_freq.pdf",
        figures_dir=FIGURES_LEGACY_DIR,
    )
    ...
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", cols_no_diag,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, labels_no_diag,
        "legacy_cluster_allgenes_or.pdf",
        figures_dir=FIGURES_LEGACY_DIR, tables_dir=TABLES_LEGACY_DIR,
    )
    ...
    if os.path.exists(PGS_FILE):
        ...
        if len(df_ors) > 0:
            ...
            df_ors.to_csv(
                os.path.join(TABLES_LEGACY_DIR, "legacy_pgs_or_clusters.csv"), index=False
            )
            plot_pgs_forest(df_ors, PALETTE_CLUSTERS, LABELS_PGS,
                            "legacy_cluster_pgs_or.pdf", FIGURES_LEGACY_DIR)
        else:
            print("  No PGS OR results to plot.")
    else:
        print(f"Warning: PGS file not found ({PGS_FILE}), skipping")

    print(f"\n=== Done. Outputs in: {FIGURES_LEGACY_DIR} ===")
