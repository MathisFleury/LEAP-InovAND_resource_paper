#!/usr/bin/env python3
# =============================================================================
# 03 - Carrier Frequencies & Odds Ratios by Legacy Cluster (NT restricted to C1)
# =============================================================================
# Same as 01_legacy_clusters.py but uses only NT subjects whose clinical
...
# Output: cluster-level figures/tables in outputs/figures_legacy_nt_c1/
# =============================================================================
...
from _config import (
    CARRIER_ANNOTATIONS, CLUSTERS_LEGACY_FILE,
    INDIVIDUALS_METRICS, DIAG_FILE, PGS_FILE,
    FIGURES_LEGACY_NTC1_DIR, TABLES_LEGACY_NTC1_DIR,
...
os.makedirs(FIGURES_LEGACY_NTC1_DIR, exist_ok=True)
os.makedirs(TABLES_LEGACY_NTC1_DIR, exist_ok=True)
...
def main():
    ...
    # --- Build Cluster_with_NT: NT only if their cluster assignment is C1 ---
    ...
    plot_carrier_frequencies(
        ...
        "legacy_nt_c1_constraint_freq.pdf", ylim=(0, 1),
        figures_dir=FIGURES_LEGACY_NTC1_DIR,
    )
    print("\n=== Cluster-level odds ratios (constrained, NT=C1) ===")
    compute_and_plot_odds_ratios(
        df, "Cluster_with_NT", COLS_DELLOF_CONSTRAINED,
        PALETTE_CLUSTERS, ORDER_CLUSTERS, LABELS_CONSTRAINED,
        "legacy_nt_c1_constraint_or.pdf",
        figures_dir=FIGURES_LEGACY_NTC1_DIR, tables_dir=TABLES_LEGACY_NTC1_DIR,
    )
    ...
    plot_carrier_frequencies(
        ...
        "legacy_nt_c1_allgenes_freq.pdf",
        figures_dir=FIGURES_LEGACY_NTC1_DIR,
    )
    ...
    if os.path.exists(PGS_FILE):
        ...
        if len(df_ors) > 0:
            ...
            df_ors.to_csv(os.path.join(TABLES_LEGACY_NTC1_DIR, "legacy_nt_c1_pgs_or.csv"), index=False)
            plot_pgs_forest(df_ors, PALETTE_CLUSTERS, LABELS_PGS,
                            "legacy_nt_c1_pgs_or.pdf", FIGURES_LEGACY_NTC1_DIR)
    else:
        print(f"Warning: PGS file not found ({PGS_FILE}), skipping")

    print(f"\n=== Done. Outputs in: {FIGURES_LEGACY_NTC1_DIR} ===")
