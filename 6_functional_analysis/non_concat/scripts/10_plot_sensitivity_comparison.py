...
SUMMARY_CSV = REPO / '6_functional_analysis' / 'non_concat' / 'outputs' / 'tables' / 'sensitivity' / 'primary_vs_sensitivity_summary.csv'
OUT_DIR = REPO / '6_functional_analysis' / 'non_concat' / 'outputs' / 'figures' / 'sensitivity'
...
def plot_sample_sizes(df: pd.DataFrame, out_path: Path) -> None:
    ...
    def _autism_nt_sizes(analysis: str, variant: str) -> tuple[float, float]:
        ...
        if variant == '6min_cutoff':
            p = REPO / '6_functional_analysis' / 'non_concat' / 'outputs' / 'tables' / 'sensitivity' / 'autism_vs_td_6min_summary.csv'
            if p.exists():
                s = pd.read_csv(p)
                return (float(s['n_autism'].iloc[0]), float(s['n_nt'].iloc[0]))
        if variant == 'covariate_adjusted':
            p = REPO / '6_functional_analysis' / 'non_concat' / 'outputs' / 'tables' / 'sensitivity' / 'autism_vs_td_covadj_summary.csv'
            if p.exists():
                s = pd.read_csv(p)
                return (float(s['n_autism'].iloc[0]), float(s['n_nt'].iloc[0]))
        return (np.nan, np.nan)
